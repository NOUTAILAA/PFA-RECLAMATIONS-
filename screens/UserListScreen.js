...
const UserListScreen = () => {
  ...
  const fetchUsers = async () => {
    try {
      const response = await fetch('http://192.168.0.112:8061/users');
      if (!response.ok) {
  ...
  const handleDeleteUser = async (id) => {
    try {
      const response = await fetch(`http://192.168.0.112:8061/user/${id}`, {
        method: 'DELETE',
      });
      if (!response.ok) {
  ...
  const handleAddUser = async () => {
    ...
    try {
      const response = await fetch('http://192.168.0.112:8061/user', {
        method: 'POST',
        headers: {
