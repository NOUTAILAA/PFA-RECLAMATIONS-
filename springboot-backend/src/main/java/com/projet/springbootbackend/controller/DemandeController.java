...
public class DemandeController {
    ...
    public ResponseEntity<?> createDemandForUser(@RequestBody demand demand, @PathVariable Long userId) {
 ...
 // Modifier l'état d'une demande en "rejetée"
    @PutMapping("/demands/{id}/reject")
    public ResponseEntity<demand> rejectDemand(@PathVariable Long id) {
        return demandRepository.findById(id)
            .map(existingDemand -> {
                existingDemand.setEtat("rejetée");
                demand updatedDemand = demandRepository.save(existingDemand);
                return ResponseEntity.ok().body(updatedDemand);
            })
            .orElseGet(() -> ResponseEntity.notFound().build());
    ...
    public ResponseEntity<demand> acceptDemand(@PathVariable Long id) {
        return demandRepository.findById(id)
                .map(existingDemand -> {
                    existingDemand.setEtat("Demande acceptée");
                    demand updatedDemand = demandRepository.save(existingDemand);
                    return ResponseEntity.ok().body(updatedDemand);
                })
                .orElseGet(() -> ResponseEntity.notFound().build());
    }
    @PutMapping("/demands/{id}/refuse")
    public ResponseEntity<demand> refuseDemand(@PathVariable Long id) {
        return demandRepository.findById(id)
                .map(existingDemand -> {
                    existingDemand.setEtat("demande refusée");
                    demand updatedDemand = demandRepository.save(existingDemand);
                    return ResponseEntity.ok().body(updatedDemand);
                })
                .orElseGet(() -> ResponseEntity.notFound().build());
